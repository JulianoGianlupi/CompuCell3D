...
class CC3DXMLGenerator:
    ...
    def generateAdhesionFlexPlugin(self):
        ...
        for id1 in range(0, maxId + 1):
            for molecule in self.afMolecules:
                afPluginElement.ElementCC3D("AdhesionMoleculeDensity",
    ...
    def generateSurfaceFlexPlugin(self):
        ...
        self.cc3d.addComment(
            "For more flexible specification of the constraint (done in Python) please use SurfaceLocalFlex plugin")
    ...
    def generateLengthConstraintPlugin(self):
        ...
        self.cc3d.addComment(
            "LengthConstrainLocalFlex allows constrain specification for each cell individually but currently works only in 2D")
    ...
    def generateLengthConstraintLocalFlexPlugin(self):
        self.cc3d.addComment("newline")
        self.cc3d.addComment(
            "Applies elongation constraint to each cell. Users specify the length major axis -TargetLength and a strength of the constraint -LambdaLength. Parameters are specified for each cell individually")
        self.cc3d.addComment(
            "IMPORTANT: To prevent cell fragmentation for large elongations you need to also use connectivity constraint")
        self.cc3d.addComment(
            "This plugin currently works only in 2D. Use the following Python syntax to set/modify length constraint:")
        self.cc3d.addComment(
    ...
    def generateChemotaxisPlugin(self):
        ...
        for chemFieldName, chemDictList in self.chemotaxisTable.iteritems():
            chemFieldElement = chemotaxisPluginElement.ElementCC3D("ChemicalField",
                                                                   {"Source": solverDict[chemFieldName],
                                                                    "Name": chemFieldName})
        ...
        for solver, fieldNames in solverDict.iteritems():
            if solver == 'KernelDiffusionSolver':
                ...
                for fieldName in fieldNames:
                    ...
                    concFieldNameElem = diffData.ElementCC3D("ConcentrationFileName", {},

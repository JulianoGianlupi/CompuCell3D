...
void DiffusionData::initialize(Automaton *_automaton){
	...
	for(std::set<std::string>::iterator sitr = avoidDecayInTypeNameSet.begin() ; sitr!=avoidDecayInTypeNameSet.end(); ++sitr){
	...
	for(std::map<std::string,float>::iterator mitr = decayCoefTypeNameMap.begin() ; mitr!=decayCoefTypeNameMap.end(); ++mitr){
		unsigned char typeId=_automaton->getTypeId(mitr->first);
		decayCoef[typeId]=mitr->second;		
	}

	for(std::map<std::string,float>::iterator mitr = diffCoefTypeNameMap.begin() ; mitr!=diffCoefTypeNameMap.end(); ++mitr){
		unsigned char typeId=_automaton->getTypeId(mitr->first);
...
void SecretionData::SecretionOnContact(std::string _secretingTypeName, std::string _onContactWithTypeName,float _secretionConst)
{
...
void SecretionData::initialize(Automaton *_automaton){
	...
	for (std::map<std::string,SecretionOnContactData>::iterator mitr = typeNameSecrOnContactDataMap.begin() ; mitr !=typeNameSecrOnContactDataMap.end() ; ++mitr){
		unsigned char typeId=_automaton->getTypeId(mitr->first);
